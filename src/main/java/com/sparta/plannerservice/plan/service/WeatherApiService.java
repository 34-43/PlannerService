...
import com.sparta.plannerservice.plan.dto.WeatherApiFetchDto;
...
public class WeatherApiService {
    ...
    public String fetchWeather(LocalDate date) {
        // 정적인 API 경로 정의
        ...
        // 전체 반환값을 Dto 리스트로 전환
        List<WeatherApiFetchDto> response = restTemplate.exchange(uri, HttpMethod.GET, null, new ParameterizedTypeReference<List<WeatherApiFetchDto>>() {
        }).getBody();
        ...
        WeatherApiFetchDto today = response.stream().filter(w -> w.getDate().equals(dateString)).findFirst().orElseThrow(() -> new FailedRequestException(FailedRequest.API_WEATHER_MALFUNCTION));
